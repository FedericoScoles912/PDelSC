...
/**
 * Obtiene todos los alumnos desde la API usando Axios
 * @returns {Promise<Array>} Lista de alumnos
 */
async function axiosGetAlumnos() {
  const response = await axios.get(API_URL);
  return response.data.data;
}

/**
 * Crea un nuevo alumno en la API usando Axios
 * @param {object} alumno - Datos del alumno
 * @returns {Promise<object>} Alumno creado
 */
async function axiosCreateAlumno(alumno) {
  const response = await axios.post(API_URL, alumno);
  return response.data;
}

/**
 * Actualiza un alumno existente usando Axios
 * @param {number} id - ID del alumno
 * @param {object} alumno - Datos del alumno
 * @returns {Promise<object>} Alumno actualizado
 */
async function axiosUpdateAlumno(id, alumno) {
  const response = await axios.put(`${API_URL}/${id}`, alumno);
  return response.data;
}

/**
 * Elimina un alumno por ID usando Axios
 * @param {number} id - ID del alumno
 * @returns {Promise<object>} Confirmación
 */
async function axiosDeleteAlumno(id) {
...
function renderTable() {
  ...
  alumnos.forEach(alumno => {
    ...
    tdAcciones.appendChild(deleteConfirm);
...
async function loadAlumnos() {
  try {
    ...
    tableContainer.innerHTML = '';
    alumnos = await axiosGetAlumnos();
    renderTable();
  } catch (error) {
    if (error.code === 'ERR_NETWORK') {
      showToast('No se pudo conectar con la API. Asegúrate de que el servidor esté corriendo en http://localhost:3000', 'error');
    } else if (error.response && error.response.data) {
      showToast(error.response.data.message || 'Error al cargar los alumnos', 'error');
    } else {
...
async function handleFormSubmit(e) {
  ...
  try {
    ...
    if (editingId) {
      await axiosUpdateAlumno(editingId, data);
      showToast('Alumno actualizado correctamente', 'success');
    } else {
      await axiosCreateAlumno(data);
    ...
    if (error.code === 'ERR_NETWORK') {
      ...
      if (errorData.field) {
        ...
        showToast(errorData.message || 'Error al guardar el alumno', 'error');
      }
    } else {
      showToast(error.message, 'error');
    }
...
async function handleDelete(id) {
  try {
    ...
    if (error.code === 'ERR_NETWORK') {
      ...
      showToast(error.response.data.message || 'Error al eliminar el alumno', 'error');
    } else {
      showToast(error.message, 'error');
    }
    console.error(error);
